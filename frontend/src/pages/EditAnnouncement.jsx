      ...
      {steps.map((step, index) => (
        <div key={step} className="flex items-center">
          <div className="flex flex-col items-center">
            ...
            <span
              className={`text-xs mt-1 hidden md:block ${index + 1 === currentStep ? "text-emerald-600 font-semibold" : "text-gray-500"}`}
            >
              {step}
            </span>
          </div>
          {index < steps.length - 1 && (
            <div
              className={`w-8 md:w-16 h-0.5 mx-1 md:mx-2 ${index + 1 < currentStep ? "bg-emerald-600" : "bg-gray-200"}`}
            />
          )}
        </div>
      ))}
...
const TypeCard = ({ icon, title, description, selected, onClick }) => (
  ...
  >
    <div
      className={`w-10 h-10 rounded-lg flex items-center justify-center ${selected ? "bg-emerald-100" : "bg-gray-100"}`}
    >
      <span className="text-lg">{icon}</span>
    </div>
    <div className="flex-1">
      <p
        className={`font-semibold text-sm ${selected ? "text-emerald-700" : "text-gray-800"}`}
      >
        {title}
      </p>
...
const ANNOUNCEMENT_TYPES = [
  {
    ...
    description: "Discounts, sales, special deals",
  },
  {
    id: "update",
    icon: "✨",
  ...
  },
  {
    ...
    icon: "🔧",
    title: "Maintenance",
    description: "Scheduled downtime or issues",
  },
  {
    id: "general",
    icon: "📢",
      ...
      if (res.data.success) {
        ...
        setFormData({
          ...
          sendEmailNotification:
            ann.priorityVisibility?.sendEmailNotification || false,
          showHomepageBanner:
            ann.priorityVisibility?.showHomepageBanner || false,
    ...
    try {
      ...
      const payload = {
        ...
        publishDate: formData.publishDate
          ? `${formData.publishDate}T12:00:00`
          : undefined,
        expiryDate: formData.expiryDate
          ? `${formData.expiryDate}T12:00:00`
          : undefined,
        priorityVisibility: {
      ...
      const res = await axios.patch(
        `${API_URL}/announcements/admin/${id}`,
        payload,
        getAuthHeader(),
      );
                  ...
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        Update the announcement message for your users
                      </p>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        Select announcement type and target audience
                      </p>
                    </div>
                    <span className="px-3 py-1 bg-emerald-100 text-emerald-700 text-xs font-semibold rounded-full">
                      Step 2 of 4
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {ANNOUNCEMENT_TYPES.map((type) => (
                          <TypeCard
                            ...
                            onClick={() =>
                              handleInputChange("type", type.title)
                            }
                          />
                        ))}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {AUDIENCES.map((aud) => (
                          <AudienceChip
                            ...
                            onClick={() =>
                              handleInputChange("targetAudience", aud.label)
                            }
                          />
                        ))}
                      </div>
                    ...
                    <div className="p-4 bg-amber-50 rounded-xl border border-amber-200">
                      <label className="flex items-start gap-3 cursor-pointer">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500 mt-1">
                            Enable this if your announcement includes a discount
                            code
                          </p>
                        </div>
                      </label>
                      {formData.isSpecialOffer && (
                        <div className="mt-4 pl-8">
                          ...
                          />
                          <p className="text-xs text-gray-400 mt-1">
                            Code will be displayed to users
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between p-4 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            Notify users via email when published
                          </p>
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    <div className="flex items-center justify-between p-4 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            Display as a prominent banner on the homepage
                          </p>
                        </div>
                      </div>
                      <ToggleSwitch
